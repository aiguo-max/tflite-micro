...
static int run_accnet_int8(uint8_t* arena) {
  printf("=== Test1: accnet_int8 (full int8, int8 I/O) ===\n");
  ...
  if (!input || input->bytes != (size_t)g_accnet_int8_input_size) {
    printf("input mismatch: got=%d expected=%d\n",
           input ? (int)input->bytes : -1, g_accnet_int8_input_size);
    return 1;
  }
  ...
  printf("\nfirst8 ref:");
  for (int i = 0; i < 8 && i < n; ++i) printf(" %d", (int)ref[i]);
  printf("\n");
